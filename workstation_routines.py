...
def assert_good_environment():
    ...
    # Ensure you are running this on a Ubuntu 16.04 server
    assert platform.dist()[0] == 'Ubuntu'
    assert platform.dist()[1].startswith('16.04')
...
### different paths:
###
### (download this repo to a temporary directory)
###     * lock (atomic operation assumed)
###     * check if hostname is already listed in fcl.json?
###         if it is:
###             make sure it is consistent with the entry in fcl.json (otherwise abort)
###         if it's not:
###             generate a host file
###             make sure no conflict (private-ip, public-ip-port) with existing entries in fcl.json (otherwise abort)
###             upload the new host file to gist.github.com and report a short URL, ask admin to write it down
###     * check fcl.json users information applicability
###     * apply fcl.json users information and hosts information (for host-based authentication)
###     * unlock
### (optionally delete this repo)
###
### I have to write a bash/python script to
### download and invoke this python script with different path (argument)
###
...
# disable apt-daily.timer and apt-daily.service or not?
# bash# apt-get update
# bash# tasksel install standard openssh-server
# bash# apt-get -y dist-upgrade
# bash# apt-get -y autoremove




'''
1. create temporary directory $T
2. change directory into $T
3. download fcl.json and workstation_routines.py
4. path/to/workstation_routines.py lock
5. path/to/workstation_routines.py check-this-host-in-json
6. path/to/workstation_routines.py check-json-applicability
7. path/to/workstation_routines.py apply-json
8. path/to/workstation_routines.py unlock
9. leave and remove directory $T

test on new virtual machines
updating fcl.json
